...
M.templates.blockTemplates = {
	paragraphTpl: function(arg){
		//console.log(arg, $.el.p({name: arg.name, class:arg.class}, $.el.div(arg.content.resource)));
		return $.el.p({name: arg.name, class:arg.class}, arg.content.resource);
	},
	titleTpl: function(arg){
		//console.log(arg);
		return $.el.h1({name: arg.name, class:arg.class}, arg.content.resource);
	},
	headingTpl: function(arg){
		return $.el.h2({name: arg.name, class:arg.class}, arg.content.resource);
	},
	imgTpl: function(arg){
		//console.log(arg, "image");
		return $.el.img({src: arg.content.resource, name: arg.id, class:arg.class});
	},
	bquoteTpl: function(arg){
		return $.el.blockquote({name: arg.name, class:arg.class}, arg.content.resource);
	},
	linkTpl: function(arg){
		return $.el.a({name: arg.name, href: arg.content.resource, class:arg.class}, arg.content.resource);
	},
	galleryTpl: function(arg){
		var src = arg.content.resource;
		var oneOrMany = src.split(',');

		return $.el.ul({name: arg.name, "data-type": "slide", class:arg.class}, (function() {
	...
	audioTpl: function(arg) {
		return $.el.div({name: arg.name, class:"audio-feed"}, arg.content.resourceCaption, (function(){
			return $.el.span({name: arg.name, class:arg.class})
		})());
	},
	labelTpl: function(arg) {
		return $.el.label({name: arg.name, class:arg.class}, arg.content.resource);
	},
	feedTpl: function(arg){
		return $.el.div({name: arg.name, class: "feed", "data-src": arg.content.resource}, 
				$.el.img({src: "assets/images/thumbnails/"+arg.content.resourceThumbnail, class: "img-responsive"}),
				$.el.p({class: "feed-title"},arg.content.resourceCaption));
	}
};
...
M.views.pageView.prototype = {
	...
	templateResolver: function(type){
		...
		switch(type){
			...
			case 'label':
			return M.templates.blockTemplates.labelTpl;
			break;

			case 'block:audio':
			return M.templates.blockTemplates.audioTpl;
			break;

			case 'feed':
			return M.templates.blockTemplates.feedTpl;
			break;
	...
	compileBlock: function(block, arg){
		...
		return compiledBlock = block.el({name: arg.id, class: block.area, content: arg});
	},
	render: function(){
		var self = this;
		...
		self.story.forEach(function(item, index){ 
			...
			//console.log(block, resource, self.layout.addBlocks(self.templateResolver(resource.type)), "resource");
			var compiledBlock, blockIfArray;
			if(block){
				...
				if(compiledBlock){
					console.log(index, compiledBlock, item);
					self.catalog.$el[item.ref] = compiledBlock
					self.$el.appendChild(compiledBlock);
				}
		...
		console.log(self.layout, self.catalog.$el);
